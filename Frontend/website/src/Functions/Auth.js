...
export const AuthProvider = ({ children }) => {
    ...
    let requestOptionsUserName = (token) => {
        return {
            method: "GET",
            credentials: "include",
    ...
    let requestOptionsUserTable = (token) => {
        return {
            method: "POST",
            credentials: "include",
            headers: {
    ...
    let getUserName = (token) => {
        fetch("http://" + dns, requestOptionsUserName(token))
    ...
    let getUserTable = (token) => {
        fetch("http://" + dns + "/users", requestOptionsUserTable(token))
            .then((response) => response.json())
            .then((data) => {
                setUserTable(data);
            })
    ...
    let logout = () => {
        fetch("http://" + dns + "/logout", requestOptionsDelete)
            .then((response) => response.json())
            .then((dataRes) => {
    ...
    let updateAccessToken = () => {
        fetch("http://" + dns + "/token", requestOptionsToken)
            .then((response) => response.json())
            .then((data) => {
